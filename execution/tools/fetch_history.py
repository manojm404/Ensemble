...
def fetch_history(limit=5, offset=0, order="DESC", company_id="company_alpha"):
    ...
    """
    audit = AuditLogger()
    history = audit.get_history(company_id, limit=int(limit), offset=int(offset), order=order)
    ...
    for event in history:
        ...
        if a_type == 'USER_INPUT':
            ...
            output.append(f"[{ts}] INTERNAL ({a_type}): {agent} performed {details.get('type', 'action')}")
