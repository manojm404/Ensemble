...
import { listCompanyActivity, type CompanyActivityRecord } from "@/lib/api";
...
export default function CompanyActivity() {
  ...
  const [activity, setActivity] = useState<CompanyActivityRecord[]>([]);

  const companyId = id || currentCompany?.id || "";
  const company = getCompanyById(companyId) || currentCompany;
  const scrollRef = useScrollMemory(`esemble_scroll_company_${companyId || "unknown"}_activity`);

  useEffect(() => {
    if (!companyId) return;
    listCompanyActivity(companyId).then(setActivity).catch(console.error);
  }, [companyId]);

  if (!company) return <div className="flex items-center justify-center h-full text-muted-foreground">No company selected</div>;

  const filtered = filter === "all" ? activity : activity.filter(e => e.action_type.includes(filter));
      ...
      <div className="flex items-center justify-between px-8 py-6 border-b border-border/40 bg-card/30 backdrop-blur-sm">
        ...
        <div className="flex items-center gap-1.5">
          {["all", "agent", "issue", "alert", "deploy", "system"].map(t => (
            <button key={t} onClick={() => setFilter(t)} className={`px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider transition-all ${filter === t ? "bg-primary/20 text-primary border border-primary/20" : "text-muted-foreground/50 hover:text-foreground hover:bg-secondary/50"}`}>{t}</button>
          ))}
        </div>
      </div>

      <div ref={scrollRef} className="flex-1 overflow-auto px-8 py-8">
        <div className="max-w-3xl mx-auto space-y-4">
          {filtered.map((event, i) => {
            const config = typeConfig[event.action_type.split(".")[0]] || typeConfig.system;
            return (
              <motion.div key={event.id} initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.02 }} className="flex items-start gap-4">
                ...
                <div className="pb-6 min-w-0 flex-1">
                  ...
                  <p className="text-[10px] text-muted-foreground/50 mt-0.5">{new Date(event.created_at).toLocaleString()} · {config.label}</p>
                </div>
              </motion.div>
            );
          })}
