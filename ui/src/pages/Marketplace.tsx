...
const Marketplace = () => {
  ...
  const handleInstall = async (pack: MarketplacePack) => {
    ...
    try {
      await installPack(pack.id, pack.download_url, pack.version);
      toast.success(`${pack.name} integrated into your local fleet.`);
      setInstalledPackIds(prev => new Set(prev).add(pack.id));
      loadData(); // Refresh to catch any new states
    } catch (err) {
      toast.error("Installation failed. Check network or manifest.");
    } finally {
