...
class AgentCreate(BaseModel):
    ...
    instruction: Optional[str] = Field(
        None, description="Agent system prompt/instructions"
    )
...
class TokenGrantRequest(BaseModel):
    ...
    workflow_id: Optional[str] = Field(
        None, description="Workflow ID this grant is for"
    )
...
class BudgetSummary(BaseModel):
    ...
    sop_run_limit: int = Field(
        ..., description="Monthly SOP run limit (100 for free tier)"
    )
...
class FeedbackCreate(BaseModel):
    ...
    category: Optional[str] = Field(
        None, max_length=100, description="Feedback category"
    )
...
class FeedbackResponse(BaseModel):
    ...
    status: str = Field(
        default="new",
        description="new, acknowledged, in_progress, resolved, rejected, planned",
    )
...
class BugReportCreate(BaseModel):
    ...
    description: str = Field(
        ..., min_length=1, max_length=10000, description="Detailed bug description"
    )
    ...
    screenshot_url: Optional[str] = Field(
        None, description="URL to screenshot of the bug"
    )
...
class HealthResponse(BaseModel):
    ...
    supabase_connected: bool = Field(
        ..., description="Whether Supabase connection is working"
    )
