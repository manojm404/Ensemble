...
Usage:
    from backend.ensemble.audit import audit_logger
...
class AuditLogger:
    ...
    def _log_supabase(
        ...
        try:
            ...
            logger.debug(
                "📝 [AuditLogger] Logged %s event to Supabase (user: %s)",
                action_type,
                user_id,
            )
            ...
            logger.warning(
                "⚠️ [AuditLogger] Failed to write to Supabase: %s — falling back to SQLite",
                e,
            )
    ...
    def _broadcast(
        ...
        logger.debug(
            "📡 [AuditLogger] Broadcasting %s event %s to %s",
            action_type,
            event_id,
            company_id,
        )
    ...
    def _replay_supabase(self, event_id):
        ...
        try:
            ...
            client = supabase_admin.client
            result = (
                client.table("audit_events").select("*").eq("id", event_id).execute()
            )
            if not result.data:
                return None
            r = result.data[0]
                ...
                if loop.is_running():
                    asyncio.run_coroutine_threadsafe(
                        ws_manager.broadcast(
                            company_id,
                            "NOTIFICATION",
                            {"title": title, "preview": preview, "category": category},
                        ),
                        loop,
                    )
                else:
    ...
    def get_notifications(
        self, user_id: str, company_id: str = None, limit: int = 50
    ) -> List[Dict[str, Any]]:
    ...
    def mark_notification_read(self, notification_id: int):
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.execute(
                "UPDATE notifications SET is_unread = 0 WHERE id = ?",
                (notification_id,),
            )
