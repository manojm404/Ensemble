...
class PrepareDocuments(Action):
    ...
    async def run(self, with_messages, **kwargs):
        ...
        user_requirements = [
            i for i in with_messages if i.cause_by == any_to_str(UserRequirement)
        ]
        if not self.config.project_path and user_requirements and self.key_descriptions:
            args = await user_requirements[0].parse_resources(
                llm=self.llm, key_descriptions=self.key_descriptions
            )
        ...
        await repo.docs.save(
            filename=REQUIREMENT_FILENAME, content=with_messages[0].content
        )
            ...
            instruct_content=AIMessage.create_instruct_value(
                kvs={
                    ...
                    "requirements_filename": str(
                        repo.docs.workdir / REQUIREMENT_FILENAME
                    ),
                    "prd_filenames": [
                        str(repo.docs.prd.workdir / i) for i in repo.docs.prd.all_files
                    ],
                },
