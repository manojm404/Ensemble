...
class WritePRD(Action):
        ...
        **kwargs,
        ...
        if not self.input_args:
            ...
            await self.repo.docs.save(
                filename=REQUIREMENT_FILENAME, content=with_messages[-1].content
            )
            self.input_args = AIMessage.create_instruct_value(
                kvs={
                    "project_path": self.context.kwargs.project_path,
                    "requirements_filename": str(
                        self.repo.docs.workdir / REQUIREMENT_FILENAME
                    ),
                    "prd_filenames": [
                        str(self.repo.docs.prd.workdir / i)
                        for i in self.repo.docs.prd.all_files
                    ],
                },
        ...
        kvs["changed_prd_filenames"] = [
            str(self.repo.docs.prd.workdir / i)
            for i in list(self.repo.docs.prd.changed_files.keys())
        ]
        kvs["project_path"] = str(self.repo.workdir)
        kvs["requirements_filename"] = str(
            self.repo.docs.workdir / REQUIREMENT_FILENAME
        )
    ...
    async def _new_prd(self, requirement: str) -> ActionNode:
        ...
        context = CONTEXT_TEMPLATE.format(
            requirements=requirement, project_name=project_name
        )
    ...
    async def _merge(self, req: Document, related_doc: Document) -> Document:
        ...
        node = await REFINED_PRD_NODE.fill(
            req=prompt, llm=self.llm, schema=self.prompt_schema
        )
    ...
    async def _save_competitive_analysis(
        self, prd_doc: Document, output_filename: Path = None
    ):
    ...
    async def _execute_api(
        ...
        async with DocsReporter(enable_llm_stream=True) as reporter:
            ...
            output_pathname = rectify_pathname(
                path=output_pathname, default_filename="prd.json"
            )
            await awrite(filename=output_pathname, data=new_prd.content)
            competitive_analysis_filename = (
                output_pathname.parent / f"{output_pathname.stem}-competitive-analysis"
            )
            await self._save_competitive_analysis(
                prd_doc=new_prd, output_filename=Path(competitive_analysis_filename)
            )
            md_output_filename = output_pathname.with_suffix(".md")
            await save_json_to_markdown(
                content=new_prd.content, output_filename=md_output_filename
            )
