...
class ImportRepo(Action):
    ...
    async def _create_repo(self):
        path = await git_clone(
            url=self.repo_path, output_dir=self.config.workspace.path
        )
    ...
    async def _create_prd(self):
        action = ExtractReadMe(
            i_context=str(self.context.repo.workdir), context=self.context
        )
        await action.run()
        graph_repo_pathname = (
        ...
        await self.repo.docs.prd.save(
            filename=self.rid + ".json", content=json.dumps(prd)
        )

    async def _create_system_design(self):
        action = RebuildClassView(
            name="ReverseEngineering",
            i_context=str(self.context.src_workspace),
        ...
        entries = []
        src_workspace = self.context.src_workspace.relative_to(
            self.context.repo.workdir
        )
        for r in rows:
            if tag in r.subject:
        ...
        full_path = RebuildSequenceView.get_full_filename(
            self.context.repo.workdir, main_entry
        )
    ...
    async def _save_system_design(self):
        ...
        file_list = self.context.git_repo.get_files(
            relative_path=".", root_relative_path=self.context.src_workspace
        )
        data = {
        ...
        await self.context.repo.docs.system_design.save(
            filename=self.rid + ".json", content=json.dumps(data)
        )
