...
class WriteDesign(Action):
        ...
        **kwargs,
        ...
        return AIMessage(
            ...
            instruct_content=AIMessage.create_instruct_value(
                kvs=kvs, class_name="WriteDesignOutput"
            ),
            cause_by=self,
        )
    ...
    async def _new_system_design(self, context):
        node = await DESIGN_API_NODE.fill(
            req=context, llm=self.llm, schema=self.prompt_schema
        )
        return node
    ...
    async def _update_system_design(self, filename) -> Document:
        ...
        old_system_design_doc = await self.repo.docs.system_design.get(
            root_relative_path.name
        )
        async with DocsReporter(enable_llm_stream=True) as reporter:
            ...
            await reporter.async_report(
                self.repo.workdir / md.root_relative_path, "path"
            )
        return doc
    ...
    async def _save_seq_flow(self, design_doc, output_filename: Path = None):
        ...
        if not seq_flow:
            return
        pathname = output_filename or self.repo.workdir / Path(
            SEQ_FLOW_FILE_REPO
        ) / Path(design_doc.filename).with_suffix("")
    ...
    async def _execute_api(
        ...
        prd_content = ""
        if prd_filename:
            prd_filename = rectify_pathname(
                path=prd_filename, default_filename="prd.json"
            )
        ...
        async with DocsReporter(enable_llm_stream=True) as reporter:
            ...
            if not legacy_design_filename:
                ...
                design = await self._merge(
                    prd_doc=Document(content=context),
                    system_design_doc=Document(content=old_design_content),
                )
            ...
            output_pathname = rectify_pathname(
                path=output_pathname, default_filename="system_design.json"
            )
            await awrite(filename=output_pathname, data=design.content)
            output_filename = (
                output_pathname.parent / f"{output_pathname.stem}-class-diagram"
            )
            await self._save_data_api_design(
                design_doc=design, output_filename=output_filename
            )
            output_filename = (
                output_pathname.parent / f"{output_pathname.stem}-sequence-diagram"
            )
            await self._save_seq_flow(
                design_doc=design, output_filename=output_filename
            )
            md_output_filename = output_pathname.with_suffix(".md")
            await save_json_to_markdown(
                content=design.content, output_filename=md_output_filename
            )
