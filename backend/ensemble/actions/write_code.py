...
class WriteCode(Action):
    ...
    async def run(self, *args, **kwargs) -> CodingContext:
        ...
        if not coding_context.code_plan_and_change_doc:
            coding_context.code_plan_and_change_doc = (
                await self.repo.docs.code_plan_and_change.get(
        ...
        if coding_context.design_doc and coding_context.design_doc.filename:
            summary_doc = await self.repo.docs.code_summary.get(
                filename=coding_context.design_doc.filename
            )
        logs = ""
        if test_doc:
        ...
        if self.config.inc:
            prompt = REFINED_TEMPLATE.format(
                ...
                code_plan_and_change=(
                    coding_context.code_plan_and_change_doc.content
                    if coding_context.code_plan_and_change_doc
                    else ""
                ),
                design=(
                    coding_context.design_doc.content
                    if coding_context.design_doc
                    else ""
                ),
        ...
        async with EditorReporter(enable_llm_stream=True) as reporter:
            await reporter.async_report(
                {"type": "code", "filename": coding_context.filename}, "meta"
            )
    ...
    async def get_codes(
        ...
        code_filenames = (
            m.get(TASK_LIST.key, [])
            if not use_inc
            else m.get(REFINED_TASK_LIST.key, [])
        )
        codes = []
