...
class WriteTasks(Action):
    ...
    async def run(
        ...
        **kwargs,
        ...
        changed_tasks = [
            str(self.repo.docs.task.workdir / i)
            for i in list(self.repo.docs.task.changed_files.keys())
        ]
        ...
        kvs["python_package_dependency_filename"] = str(
            self.repo.workdir / PACKAGE_REQUIREMENTS_FILENAME
        )
        return AIMessage(
            ...
            instruct_content=AIMessage.create_instruct_value(
                kvs=kvs, class_name="WriteTaskOutput"
            ),
            cause_by=self,
        )
    ...
    async def _update_tasks(self, filename):
        ...
        system_design_doc = await Document.load(
            filename=filename, project_path=self.repo.workdir
        )
    ...
    async def _merge(self, system_design_doc, task_doc) -> Document:
        ...
        node = await REFINED_PM_NODE.fill(
    ...
    async def _execute_api(
        ...
        if design_filename:
            design_filename = rectify_pathname(
                path=design_filename, default_filename="system_design.md"
            )
        ...
        async with DocsReporter(enable_llm_stream=True) as reporter:
            ...
            if not output_pathname:
                output_pathname = (
                    Path(output_pathname) / "docs" / "project_schedule.json"
                )
            ...
            output_pathname = rectify_pathname(
                path=output_pathname, default_filename="project_schedule.json"
            )
            ...
            await save_json_to_markdown(
                content=file_content, output_filename=md_output_filename
            )
