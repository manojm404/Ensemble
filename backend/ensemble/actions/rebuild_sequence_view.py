...
class RebuildSequenceView(Action):
    ...
    async def run(self, with_messages=None, format=None):
        ...
        self.graph_db = await DiGraphRepository.load_from(
    ...
    async def _rebuild_main_sequence_view(self, entry: SPO):
        ...
        participants = await self._search_participants(
            split_namespace(entry.subject)[0]
        )
        class_details = []
        class_views = []
        for c in classes:
        ...
        rows = await self.graph_db.select(
            subject=entry.subject, predicate=GraphKeyword.HAS_SEQUENCE_VIEW
        )
        for r in rows:
            if r.predicate == GraphKeyword.HAS_SEQUENCE_VIEW:
                await self.graph_db.delete(
                    subject=r.subject, predicate=r.predicate, object_=r.object_
                )
        await self.graph_db.insert(
            subject=entry.subject,
            predicate=GraphKeyword.HAS_SEQUENCE_VIEW,
            object_=sequence_view,
        )
        await self.graph_db.insert(
            subject=entry.subject,
            predicate=GraphKeyword.HAS_SEQUENCE_VIEW_VER,
            object_=concat_namespace(
                datetime.now().strftime("%Y%m%d%H%M%S%f")[:-3], add_affix(sequence_view)
            ),
        )
        for c in classes:
            await self.graph_db.insert(
                subject=entry.subject,
                predicate=GraphKeyword.HAS_PARTICIPANT,
                object_=auto_namespace(c.subject),
            )
    ...
    async def _rebuild_use_case(self, ns_class_name: str):
        ...
        """
        rows = await self.graph_db.select(
            subject=ns_class_name, predicate=GraphKeyword.HAS_CLASS_USE_CASE
        )
        if rows:
            return
        ...
        for block in code_blocks:
            ...
            await self.graph_db.insert(
                ...
                predicate=GraphKeyword.HAS_CLASS_USE_CASE,
                object_=detail.model_dump_json(),
            )
    ...
    async def _get_class_use_cases(self, ns_class_name: str) -> str:
        ...
        for i, r in enumerate(rows):
            ...
            for j, use_case in enumerate(detail.use_cases):
                ...
                block += "\n##### Inputs\n" + "\n".join(
                    [f"- {s}" for s in use_case.inputs]
                )
                block += "\n##### Outputs\n" + "\n".join(
                    [f"- {s}" for s in use_case.outputs]
                )
                block += "\n##### Actors\n" + "\n".join(
                    [f"- {s}" for s in use_case.actors]
                )
                block += "\n##### Steps\n" + "\n".join(
                    [f"- {s}" for s in use_case.steps]
                )
            block += "\n#### Use Case Relationship\n" + "\n".join(
                [f"- {s}" for s in detail.relationship]
            )
        return block + "\n"
    ...
    async def _get_class_detail(self, ns_class_name: str) -> DotClassInfo | None:
        ...
        """
        rows = await self.graph_db.select(
            subject=ns_class_name, predicate=GraphKeyword.HAS_DETAIL
        )
        if not rows:
            return None
    ...
    async def _get_uml_class_view(self, ns_class_name: str) -> UMLClassView | None:
        ...
        """
        rows = await self.graph_db.select(
            subject=ns_class_name, predicate=GraphKeyword.HAS_CLASS_VIEW
        )
        if not rows:
            return None
    ...
    async def _get_source_code(self, ns_class_name: str) -> str:
        ...
        """
        rows = await self.graph_db.select(
            subject=ns_class_name, predicate=GraphKeyword.HAS_PAGE_INFO
        )
        filename = split_namespace(ns_class_name=ns_class_name)[0]
        if not rows:
            src_filename = RebuildSequenceView.get_full_filename(
    ...
    async def _search_new_participant(self, entry: SPO) -> str | None:
        ...
        rows = await self.graph_db.select(
            subject=entry.subject, predicate=GraphKeyword.HAS_PARTICIPANT
        )
        merged_participants = []
        for r in rows:
    ...
    async def _merge_participant(self, entry: SPO, class_name: str):
        ...
        if len(participants) == 0:  # external participants
            await self.graph_db.insert(
                subject=entry.subject,
                predicate=GraphKeyword.HAS_PARTICIPANT,
                object_=concat_namespace("?", class_name),
            )
            return
        if len(participants) > 1:
            for r in participants:
                await self.graph_db.insert(
                    subject=entry.subject,
                    predicate=GraphKeyword.HAS_PARTICIPANT,
                    object_=auto_namespace(r.subject),
                )
            return
        ...
        await self.graph_db.insert(
            subject=entry.subject,
            predicate=GraphKeyword.HAS_PARTICIPANT,
            object_=auto_namespace(participant.subject),
        )
        await self._save_sequence_view(subject=entry.subject, content=sequence_view)
    ...
    async def _save_sequence_view(self, subject: str, content: str):
        ...
        await self.context.repo.resources.data_api_design.save(
            filename=str(filename), content=content
        )
