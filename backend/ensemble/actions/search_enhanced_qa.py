...
class SearchEnhancedQA(Action):
    ...
    collect_links_action: CollectLinks = Field(
        default_factory=CollectLinks,
        description="Action to collect relevant links from a search engine.",
    )
    ...
    per_page_timeout: float = Field(
        default=20,
        description="The maximum time for fetching a single page is in seconds. Defaults to 20s.",
    )
    java_script_enabled: bool = Field(
        default=False,
        description="Whether or not to enable JavaScript in the web browser context. Defaults to False.",
    )
    ...
    def initialize(self):
        if self.web_browse_and_summarize_action is None:
            ...
            self.web_browse_and_summarize_action = WebBrowseAndSummarize(
                web_browser_engine=web_browser_engine
            )
    ...
    async def _rewrite_query(self, query: str) -> str:
        ...
        try:
            ...
            logger.warning(
                f"Query rewrite failed. Returning original query. Error: {e}"
            )
            return query
    ...
    async def _build_context(self, query: str) -> str:
        ...
        context = "\n\n".join(
            [f"[[citation:{i+1}]] {c}" for i, c in enumerate(citations)]
        )
    ...
    async def _search_citations(self, query: str) -> list[str]:
        ...
        await self._reporter.async_report(
            {"type": "search", "stage": "searching", "urls": relevant_urls}
        )
    ...
    async def _summarize_web_content(self, urls: list[str]) -> dict[str, str]:
        ...
        await self._reporter.async_report(
            {
                "type": "search",
                "stage": "browsing",
                "pages": [i.model_dump() for i in contents],
            }
        )
        for content in contents:
            ...
            if self.web_browse_and_summarize_action._is_content_invalid(inner_text):
                logger.warning(
                    f"Invalid content detected for URL {url}: {inner_text[:10]}..."
                )
                continue
    ...
    async def _generate_answer(self, query: str, context: str) -> str:
        ...
        async with ThoughtReporter(
            uuid=self._reporter.uuid, enable_llm_stream=True
        ) as reporter:
