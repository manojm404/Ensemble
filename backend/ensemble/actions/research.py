...
class CollectLinks(Action):
    ...
    async def run(
        ...
        prompt = reduce_message_length(
            gen_msg(), model_name, system_text, self.config.llm.max_token
        )
        logger.debug(prompt)
    ...
    async def _search_and_rank_urls(
        ...
        prompt = COLLECT_AND_RANKURLS_PROMPT.format(
            topic=topic, query=query, results=_results, time_stamp=time_stamp
        )
        logger.debug(prompt)
    ...
    async def _search_urls(self, query: str, max_results: int) -> list[dict[str, str]]:
        ...
        return await self.search_engine.run(
...
class WebBrowseAndSummarize(Action):
    ...
    async def run(
        ...
        *urls: str,
        ...
        """
        contents = await self._fetch_web_contents(
            url, *urls, per_page_timeout=per_page_timeout
        )

        all_urls = [url] + list(urls)
        summarize_tasks = [
            self._summarize_content(content, query, system_text) for content in contents
        ]
        summaries = await self._execute_summarize_tasks(
            summarize_tasks, use_concurrent_summarization
        )
    ...
    async def _fetch_web_contents(
        ...
        contents = await self.web_browser_engine.run(
            url, *urls, per_page_timeout=per_page_timeout
        )
    ...
    async def _summarize_content(
        self, page: WebPage, query: str, system_text: str
    ) -> str:
        ...
        try:
            ...
            if self._is_content_invalid(content):
                logger.warning(
                    f"Invalid content detected for URL {page.url}: {content[:10]}..."
                )
                return None
    ...
    async def _execute_summarize_tasks(
        self, tasks: list[Coroutine[Any, Any, str]], use_concurrent: bool
    ) -> list[str]:
