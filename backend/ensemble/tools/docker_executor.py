...
from backend.ensemble.network_governance import NetworkGovernor
...
class DockerExecutor:
    ...
    def check_availability(self):
        ...
        try:
            ...
            logging.info(
                "🐳 DockerExecutor: Daemon reached. Sovereign Sandboxing & Networking ENABLED."
            )
        except Exception as e:
            DOCKER_AVAILABLE = False
            logging.error(
                f"⚠️ DockerExecutor: Daemon unreachable or permission denied: {e}. Sovereign Sandboxing DISABLED."
            )
    ...
    def run_container(
        ...
        if not DOCKER_AVAILABLE:
            return {
                "status": "error",
                "message": "Docker not available. Falling back to unprotected mode.",
            }
        ...
        try:
            ...
            net_cfg = self.network_gov.get_container_config(
                os.getenv("AGENT_ID", "default_agent")
            )
