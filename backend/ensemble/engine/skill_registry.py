...
class SkillRegistry:
    ...
    def sync_all(self):
        ...
        print(
            f"   📂 Native: {self.native_dir} (exists: {os.path.exists(self.native_dir)})"
        )
        print(
            f"   📂 Legacy: {self.legacy_skills_dir} (exists: {os.path.exists(self.legacy_skills_dir)})"
        )
        print(
            f"   📂 Custom: {self.custom_dir} (exists: {os.path.exists(self.custom_dir)})"
        )
        ...
        try:
            ...
            print(
                f"✅ [SkillRegistry] Successfully loaded {len(self.skills)} specialist agents."
            )
    ...
    def _parse_native_agent(
        self, filepath: str, source: SkillSource, format: AgentFormat
    ):
    ...
    def _get_emoji_for_format(self, format: AgentFormat) -> str:
        if format == AgentFormat.PYTHON:
            return "🐍"
        if format == AgentFormat.YAML:
            return "📄"
        if format == AgentFormat.JSON:
            return "📋"
        if format == AgentFormat.TEXT:
            return "📝"
        return "🤖"
    ...
    def _generate_skill_id(self, filepath: str, source: SkillSource) -> str:
        ...
        if source == SkillSource.NATIVE:
            ...
            clean_id = (
                rel_to_native.replace("/", "_").replace(".md", "").replace("\\", "_")
            )
            ...
            clean_id = (
                rel_to_legacy.replace("/", "_").replace(".md", "").replace("\\", "_")
            )
            ...
            clean_id = (
                rel_to_int.replace("/", "_").replace(".md", "").replace("\\", "_")
            )
    ...
    def _is_pack_directory(self, dirname: str) -> bool:
        ...
        has_agents = os.path.isdir(agents_dir) and any(
            f.endswith(".md") for f in os.listdir(agents_dir)
        )
    ...
    def _load_integrations(self):
        ...
        for repo in os.listdir(self.integrations_dir):
            ...
            has_metagpt = os.path.exists(
                os.path.join(repo_path, "config/config.yaml")
            ) or os.path.exists(os.path.join(repo_path, "config/config2.yaml"))

            if has_superagi:
                self._hydrate_superagi(repo_path, repo)
            elif has_metagpt:
                self._hydrate_metagpt(repo_path, repo)
            else:
                self._hydrate_generic(repo_path, repo)
    ...
    def _hydrate_generic(self, path: str, repo_name: str):
        ...
        self.skills[agent_id] = {
            ...
            "source": SkillSource.INTEGRATION.value,
            "namespace": SkillSource.INTEGRATION.value,
            "category": repo_name.title(),
    ...
    def delete_skill(self, agent_id: str):
        ...
        if skill["source"] == SkillSource.PACK.value:
            raise Exception(
                "Cannot delete individual pack agents. Uninstall the entire pack instead."
            )
        ...
        return [
            {
                ...
                "source": v["source"],
                "namespace": v.get("namespace", v["source"]),
                "enabled": v["enabled"],
                "is_native": v["source"] == SkillSource.NATIVE.value,
                "pack_id": v.get("pack_id"),
                "tags": v.get("tags", []),
                ...
                exact_matches.append(
                    {
                        ...
                        "resolution": (
                            "replace" if new_agent.get("force_replace") else "conflict"
                        ),
                    }
                )
            else:
    ...
    def _find_similar_agents(
        self, new_agent: Dict[str, Any], threshold: float = 0.8
    ) -> List[Dict[str, Any]]:
                ...
                similar.append(
                    {
                        ...
                        "recommendation": (
                            "review" if max_sim < 0.9 else "likely_duplicate"
                        ),
                    }
                )
    ...
    def get_pack_agents(self, pack_id: str) -> List[Dict[str, Any]]:
        ...
        return [
            skill for skill in self.skills.values() if skill.get("pack_id") == pack_id
        ]
