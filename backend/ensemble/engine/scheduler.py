...
class SovereignScheduler:
    ...
    async def _run_job(self, job: sqlite3.Row):
        ...
        logger.info(
            f"🕒 [SovereignScheduler] Executing job '{name}' (WF: {workflow_id})"
        )

        # 1. Update status to 'running'
        with sqlite3.connect(self.audit.db_path) as conn:
            conn.execute(
                "UPDATE scheduled_jobs SET status = 'running', last_run = ? WHERE id = ?",
        ...
        try:
            ...
            from backend.ensemble.governance import governance_manager
            ...
            if wf_data and wf_data.get("graph"):
                # Run in background
                asyncio.create_task(
                    self.dag_engine.execute_workflow(
                ...
                logger.error(
                    f"❌ [SovereignScheduler] Workflow {workflow_id} not found for job {name}"
                )
