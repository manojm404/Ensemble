...
class LifecycleManager:
    ...
    async def on_startup(self):
        ...
        try:
            from backend.ensemble.supabase_client import verify_connection
            ...
            if result["success"]:
                ...
                logger.warning(
                    "⚠️ [Lifecycle] Supabase connection failed: %s", result.get("error")
                )
        ...
        try:
            from backend.ensemble.realtime import realtime_subscriptions
    ...
    async def _save_shutdown_state(self):
        ...
        try:
            from backend.ensemble.governance import sop_runs
