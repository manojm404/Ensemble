...
class PackBuilder:
    ...
    def _build_pack(
        ...
        display = CATEGORY_DISPLAY.get(
            category, CATEGORY_DISPLAY[AgentCategory.GENERAL]
        )
    ...
    def _create_zip(self, agents: List[AgentData], zip_path: str, pack_id: str) -> int:
        """
        ...
        with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zf:
            ...
            zf.writestr(
                "pack.json", json.dumps(pack_json, indent=2, ensure_ascii=False)
            )
    ...
    def _agent_summary(self, agent: AgentData) -> Dict[str, Any]:
        ...
        return {
            ...
            "category": (
                ...
                if isinstance(agent.category, AgentCategory)
                else str(agent.category)
            ),
            "format": (
                agent.format.value
                if isinstance(agent.format, AgentFormat)
                else str(agent.format)
            ),
