...
Usage in FastAPI routes:
    ...
    from backend.ensemble.auth import get_current_user, require_auth
    from backend.ensemble.auth.models import UserInToken
...
async def verify_token_with_supabase(token: str) -> dict:
    ...
    if response.status_code == 200:
        ...
        logger.warning(
            "Supabase auth error [%d]: %s", response.status_code, response.text
        )
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
