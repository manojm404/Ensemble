...
from backend.ensemble import settings
from backend.ensemble.security.audit import AuditLogger
from backend.ensemble.api.auth import is_public_path, require_auth
from backend.ensemble.api.auth_routes import health_router
from backend.ensemble.api.auth_routes import router as auth_router
...
from backend.ensemble.integrations.supabase_client import supabase_admin

# Universal importer and pack builder
from backend.ensemble.integrations.universal_importer import universal_importer
...
GOV_CONFIG = {
    ...
    "sensitive_tools": os.getenv(
        "APPROVAL_TOOLS", "shell_cmd,python_interpreter,delete_file,send_email,deploy"
    ).split(","),
...
app.mount(
    "/static/marketplace/zips",
    StaticFiles(directory="data/marketplace/zips"),
    name="marketplace_zips",
)
...
async def auth_middleware(request: Request, call_next):
    ...
    enforce_auth = os.environ.get("ENFORCE_AUTH", "true").lower() in (
        "true",
        "1",
        "yes",
    )
    ...
    if not authorization:
        return JSONResponse(
            ...
            content={
                "status": "error",
                "error": "unauthorized",
                "message": "Authentication required. Include 'Authorization: Bearer <token>' in your request.",
            },
            headers={"WWW-Authenticate": "Bearer"},
        )

    try:
        from backend.ensemble.api.auth import extract_bearer_token, verify_token_with_supabase
        ...
        try:
            profile_result = (
                supabase_admin.client.table("profiles")
                .select("*")
                .eq("id", user_data["id"])
                .execute()
            )
...
dag_engine = DAGWorkflowEngine(
    space=space, audit=audit_logger, llm=llm, gov=None  # Will be set via gov_instance
)
...
async def startup_event():
    ...
    global scheduler
    try:
        from backend.ensemble.scheduler import init_scheduler
...
app.mount(
    "/api/workspace", StaticFiles(directory=WORKSPACE_DIR), name="workflow_workspace"
)
    ...
    try:
        if gov_instance:
            with sqlite3.connect(gov_instance.db_path) as conn:
                cursor = conn.execute(
                    "SELECT id, name, emoji, description, instruction, category, model, temperature FROM custom_agents"
                )
...
class Governance:
    ...
    def _load_pending_from_db(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute(
                "SELECT approval_id, agent_id, action, details_json, reason, timestamp FROM pending_approvals WHERE status = 'PENDING'"
            )
            for row in cursor.fetchall():
                ...
                print(
                    f"⚖️ Governance: Reloaded PENDING approval {app_id} for {agent_id}"
                )

    async def request_human_approval(
        self, agent_id: str, action: str, details: Dict[str, Any], reason: str
    ) -> bool:
        """
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            """,
                (
                    ...
                    details_json,
                    reason,
                    self.approval_data[approval_id]["timestamp"],
                ),
            )

        print(
            f"⚖️ Governance: Pending Approval {approval_id} for {agent_id} ({reason})"
        )
        ...
        try:
            ...
            print(
                f"⚖️ Governance: Decision received for {approval_id}: {'APPROVED' if result else 'DENIED'}"
            )
        except asyncio.TimeoutError:
            print(
                f"⚖️ Governance: Approval {approval_id} TIMED OUT after {GOV_CONFIG['timeout']}s"
            )
            result = False
    ...
    def request_token_grant(self, agent_id: str, estimated_cost: float) -> bool:
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute(
                "SELECT spent, escrowed, monthly_limit FROM budgets WHERE agent_id = ?",
                (agent_id,),
            )
            row = cursor.fetchone()
            if row:
                ...
                if spent + escrowed + estimated_cost <= limit:
                    conn.execute(
                        "UPDATE budgets SET escrowed = escrowed + ? WHERE agent_id = ?",
    ...
    def get_budget_status(self, agent_id: str) -> dict:
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute(
                "SELECT spent, monthly_limit FROM budgets WHERE agent_id = ?",
                (agent_id,),
            )
            row = cursor.fetchone()
            if row:
    ...
    def validate_spawn(self, parent_id: str, company_id: str, depth: int) -> bool:
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute(
                "SELECT COUNT(*) FROM agents WHERE company_id = ? AND status = 'ACTIVE'",
                (company_id,),
            )
            count = cursor.fetchone()[0]
            if count >= MAX_CONCURRENT:
                raise ValueError(
                    f"Max concurrent agents {MAX_CONCURRENT} reached for company."
                )
        return True
    ...
    def deregister_agent(self, agent_id: str):
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.execute(
                "UPDATE agents SET status = 'INACTIVE' WHERE agent_id = ?", (agent_id,)
            )
        ...
        while True:
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                for app_id, agent_id in cursor.fetchall():
                    print(
                        f"⚠️ Governance: Approval {app_id} for {agent_id} TIMED OUT after 24h."
                    )
                    conn.execute(
                        "UPDATE pending_approvals SET status = 'TIMEOUT' WHERE approval_id = ?",
                        (app_id,),
                    )
                    conn.execute(
                        "UPDATE agents SET status = 'MIA' WHERE agent_id = ?",
                        (agent_id,),
                    )
                    ...
                    audit_logger.log(
                        ...
                        {
                            "approval_id": app_id,
                            "reason": "24h manual approval timeout",
                        },
                    )
    ...
    def start_heartbeat_monitor(self):
        ...
        def monitor():
            while True:
                ...
                with sqlite3.connect(self.db_path) as conn:
                    cursor = conn.execute(
                        "SELECT agent_id, endpoint FROM agents WHERE endpoint IS NOT NULL"
                    )
    ...
    def get_macro(self, macro_id: str) -> Optional[Dict[str, Any]]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute(
                "SELECT graph_json FROM macros WHERE macro_id = ?", (macro_id,)
            )
            row = cursor.fetchone()
            if row:
...
async def create_custom_agent(req: Dict[str, Any]):
    ...
    rel_path = (
        os.path.relpath(file_path, skill_registry.custom_dir)
        .replace("/", "_")
        .replace(".md", "")
        .replace("\\", "_")
    )
...
async def import_external_repo(data: Dict[str, str]):
    ...
    try:
        ...
        return {
            "status": "success",
            "message": "Import started via Universal Importer",
            "job_id": job.job_id,
        }
    except Exception as e:
...
async def get_import_result(job_id: str):
    ...
    if not result:
        raise HTTPException(
            status_code=404, detail="Import result not found or job still running"
        )
    return result
...
async def install_imported_pack(pack_id: str, job_id: str = Query(...)):
    ...
    try:
        ...
        if not os.path.exists(zip_path):
            ...
            print(
                f"❌ [Install] ZIP not found: {pack_id}.zip. Available ZIPs: {available_zips}",
                flush=True,
            )
            raise HTTPException(
                status_code=404,
                detail=f"Pack ZIP not found: {pack_id}. Available: {', '.join(available_zips)}",
            )
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            for member in zf.namelist():
                ...
                if not any(
                    member.endswith(ext)
                    for ext in [".md", ".py", ".yaml", ".json", ".txt"]
                ):
                    continue
        ...
        print(
            f"✅ [Install] Extracted {extracted_count} files to {install_dir}",
            flush=True,
        )
        ...
        return {
            "status": "success",
            "message": f"Pack {pack_id} installed",
            "total_agents": count,
        ...
        print(
            f"❌ [Install] Installation failed: {str(e)}\n{traceback.format_exc()}",
            flush=True,
        )
        ...
        "formats": [
            {
                ...
                "description": "Native Ensemble format",
            },
            {
                "id": "python",
                "name": "Python Classes",
                "description": "MetaGPT/CrewAI roles",
            },
            {
                "id": "yaml",
                "name": "YAML Configs",
                "description": "LangChain/AutoGen configurations",
            },
            {
                "id": "json",
                "name": "JSON Manifests",
                "description": "SuperAGI/OpenAI manifests",
            },
...
async def generate_chat_response_endpoint(req: Dict[str, Any]):
    ...
    if use_skills and effective_agent_id:
        ...
        if skill:
            ...
            system_instruction = (
                f"Your specific mandate is: {skill.get('description', '')}."
            )
...
async def delete_agent(agent_id: str):
    ...
    try:
        ...
        if not success:
            raise HTTPException(
                status_code=404, detail="Agent not found or path invalid."
            )
...
async def get_workspace_preview():
    ...
    if not os.path.exists(preview_path):
        return HTMLResponse(
            content="<div style='color:#666;text-align:center;padding:40px;font-family:sans-serif;'>No web deliverable generated for this run. Check the 'Files' tab for Word/Excel/PDF artifacts.</div>"
        )
...
async def submit_decision(approval_id: str, request: Request):
    ...
    if approval_id not in gov_instance.pending_approvals:
        raise HTTPException(
            status_code=404, detail="Approval ID not found or already processed."
        )
...
async def list_workflows(search: Optional[str] = None):
    with sqlite3.connect(gov_instance.db_path) as conn:
        if search:
            cursor = conn.execute(
                "SELECT id, name, graph_json, updated_at FROM workflows WHERE name LIKE ? ORDER BY updated_at DESC",
                (f"%{search}%",),
            )
        else:
            cursor = conn.execute(
                "SELECT id, name, graph_json, updated_at FROM workflows ORDER BY updated_at DESC"
            )
        return [
...
async def get_audit_events(
    company_id: str = "company_alpha", limit: int = 50, offset: int = 0
):
...
async def run_sop(request: Request):
    ...
    if user_id:
        ...
        profile_result = supabase_admin.query(
            "profiles",
            "select",
            columns="tier,sop_run_count",
            eq="id",
            eq_value=user_id,
        )
    ...
    if yaml_content:
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to write visual SOP: {e}"
            )
    ...
    async def exec_sop():
        try:
            ...
            if input_text:
                audit_logger.log(
                    "company_alpha", "human_user", "USER_INPUT", {"text": input_text}
                )
            ...
            await ws_manager.broadcast(
                "company_alpha", "FAILURE", {"run_id": run_id, "error": str(e)}
            )
...
async def resume_sop(run_id: str):
    with sqlite3.connect(gov_instance.db_path) as conn:
        cursor = conn.execute(
            "SELECT sop_path FROM sop_runs WHERE run_id = ?", (run_id,)
        )
        row = cursor.fetchone()
        if not row:
            raise HTTPException(status_code=404, detail="Run ID not found")
...
async def generate_sop(request: GenerateRequest):
    ...
    skills_text = "\n".join(
        [f"- {s['name']}: {s['description']} (Role: {s['id']})" for s in skills]
    )
    ...
    async def try_generate(attempt=1):
        ...
        try:
            ...
            if attempt < 2:
                ...
                raise HTTPException(
                    status_code=500, detail=f"Failed to generate valid SOP: {str(e)}"
                )
...
async def update_gov_config(config: ConfigUpdate):
    if config.memory_turns is not None:
        GOV_CONFIG["memory_turns"] = config.memory_turns
    if config.cost_threshold is not None:
        GOV_CONFIG["cost_threshold"] = config.cost_threshold
    if config.timeout is not None:
        GOV_CONFIG["timeout"] = config.timeout
    return {"status": "success", "config": GOV_CONFIG}
...
async def get_pending_approvals():
    return [
        {"id": k, **v}
        for k, v in gov_instance.approval_data.items()
...
async def validate_sop(request: Request):
    try:
        ...
        if not sop.get("states"):
            errors.append("Missing 'states' block")
        if sop.get("states"):
...
async def websocket_endpoint(websocket: WebSocket, company_id: str):
    ...
    from backend.ensemble.api.auth import verify_token_with_supabase
    ...
    if not token:
        await websocket.close(
            ...
            reason="Missing token. Connect with: ws://host/ws/{company_id}?token=JWT_TOKEN",
        )
        return
    ...
    try:
        ...
        logger.info(
            "🔌 [WS] User %s connected to company %s", user["id"][:8], company_id
        )
    except HTTPException:
        await websocket.close(
            code=ws_status.WS_1008_POLICY_VIOLATION, reason="Invalid or expired token"
        )
        return
    except Exception as e:
        logger.warning("⚠️ [WS] Connection failed: %s", e)
        await websocket.close(
            code=ws_status.WS_1011_INTERNAL_ERROR, reason="Authentication failed"
        )
        return
...
async def get_chat_topics():
    with sqlite3.connect(gov_instance.db_path) as conn:
        cursor = conn.execute(
            "SELECT id, title, assistant_id, created_at, updated_at FROM chat_topics ORDER BY updated_at DESC"
        )
        return [
            {
                "id": r[0],
...
async def create_chat_topic(req: Dict[str, Any]):
    ...
    with sqlite3.connect(gov_instance.db_path) as conn:
        conn.execute(
            "INSERT INTO chat_topics (id, title, assistant_id) VALUES (?, ?, ?) ON CONFLICT(id) DO UPDATE SET title=excluded.title, assistant_id=excluded.assistant_id, updated_at=CURRENT_TIMESTAMP",
            (topic_id, title, assistant_id),
        )
...
async def get_chat_messages(topic_id: str):
    with sqlite3.connect(gov_instance.db_path) as conn:
        cursor = conn.execute(
            "SELECT id, role, content, agent_id, timestamp FROM chat_messages WHERE topic_id = ? ORDER BY timestamp ASC",
            (topic_id,),
        )
        return [
            {
...
async def save_chat_message(req: Dict[str, Any]):
    ...
    with sqlite3.connect(gov_instance.db_path) as conn:
        ...
        cursor = conn.execute(
            "SELECT COUNT(*) FROM chat_messages WHERE topic_id = ?", (topic_id,)
        )
        message_count = cursor.fetchone()[0]
        print(
            f"📝 [Messages API] Saving message to topic {topic_id}: role={role}, message_count={message_count}, content_preview={content[:30]}",
            flush=True,
        )
        ...
        if role == "user" and message_count == 0:
            ...
            conn.execute(
                "UPDATE chat_topics SET title = ?, updated_at=CURRENT_TIMESTAMP WHERE id=?",
                (title, topic_id),
            )
        else:
            conn.execute(
                "UPDATE chat_topics SET updated_at=CURRENT_TIMESTAMP WHERE id=?",
                (topic_id,),
            )

        conn.execute(
            "INSERT INTO chat_messages (id, topic_id, role, content, agent_id) VALUES (?, ?, ?, ?, ?)",
            (msg_id, topic_id, role, content, agent_id),
        )
    ...
    with sqlite3.connect(gov_instance.db_path) as conn:
        ...
        for row in cursor.fetchall():
            ...
            try:
                ...
                agent_count = (
                    len(graph.get("nodes", [])) if isinstance(graph, dict) else 2
                )
            except:
                agent_count = 2
            ...
            workflows.append(
                {
                    ...
                    "status": status,
                    "lastRun": (
                        _format_relative_time(updated_at) if updated_at else "unknown"
                    ),
                }
            )
        return workflows
        ...
        for row in cursor.fetchall():
            ...
            if graph_json:
                try:
                    ...
                    node_info = next(
                        (n for n in nodes if n.get("id") == current_node), None
                    )
                    if node_info:
                        label = node_info.get("data", {}).get("label") or node_info.get(
                            "id"
                        )
            ...
            pipelines.append(
                {
                    ...
                    "current_step_index": (
                        int(current_node.replace("n", ""))
                        if current_node
                        and current_node.startswith("n")
                        and current_node[1:].isdigit()
                        else 1
                    ),
                    "total_steps": total_steps,
                    "started_at": started_at,
                    "time": (
                        _format_relative_time(started_at) if started_at else "unknown"
                    ),
                    "iteration": curr_iter,
...
async def get_notifications(request: Request, limit: int = Query(default=50)):
    ...
    notifications = audit_logger.get_notifications(
        user_id=user["id"], company_id=company_id, limit=limit
    )
...
@app.post(
    "/api/notifications/{notification_id}/read", dependencies=[Depends(require_auth)]
)
...
async def fork_run(run_id: str, snapshot_id: int):
    ...
    with sqlite3.connect(gov_instance.db_path) as conn:
        ...
        cursor = conn.execute(
            "SELECT workflow_id FROM executions WHERE run_id = ?", (run_id,)
        )
        ...
        cursor = conn.execute(
            "SELECT graph_state_compressed FROM snapshots WHERE id = ?", (snapshot_id,)
        )
...
async def get_workflow(wf_id: str):
    ...
    with sqlite3.connect(gov_instance.db_path) as conn:
        cursor = conn.execute(
            "SELECT id, name, graph_json FROM workflows WHERE id = ?", (wf_id,)
        )
        row = cursor.fetchone()
        if not row:
...
async def list_macros():
    ...
    with sqlite3.connect(gov_instance.db_path) as conn:
        cursor = conn.execute(
            "SELECT macro_id, name, author_id, created_at FROM macros"
        )
        return [
            {"id": row[0], "name": row[1], "author": row[2], "created_at": row[3]}
            for row in cursor.fetchall()
        ]
    ...
    with sqlite3.connect(gov_instance.db_path) as conn:
        ...
        """,
            (
                macro_id,
                macro["name"],
                json.dumps(macro["graph_json"]),
                macro.get("author", "anonymous"),
...
async def get_security_policy():
    ...
    from backend.ensemble.security_policy import PERMISSIONS_FILE
...
async def run_workflow(request: Request):
    ...
    try:
        ...
        print(
            f"🚀 [Workflow Execution] Starting run {workflow_id} with {len(nodes)} agents...",
            flush=True,
        )
        ...
        for n in nodes:
            nd = n.get("data", {})
            print(
                f"  📦 Node: {nd.get('label', 'Unknown')}, role={nd.get('role', 'N/A')}, is_custom={nd.get('is_custom', False)}, instruction_len={len(nd.get('instruction', ''))}",
                flush=True,
            )

        # Initialize and Run the DAG Engine with global singletons
        engine = DAGWorkflowEngine(
            space=space, audit=audit_logger, llm=llm, gov=gov_instance
        )
        ...
        with sqlite3.connect(gov_instance.db_path) as conn:
            cursor = conn.execute(
                "SELECT node_id, status FROM node_executions WHERE run_id = ?",
                (run_id,),
            )
...
async def generate_workflow_api(request: Request):
    ...
    try:
        ...
        print(
            f"🪄 [Workflow Generation] Designing DAG for: {prompt[:50]}...", flush=True
        )
...
"""
        response = await llm.chat(
            ...
            temperature=0.1,
        )  # Lower temperature for more stable JSON
        ...
        try:
            ...
            print(
                f"❌ [Workflow Generation] JSON Parse Error: {e}\nRaw Text: {text[:500]}...",
                flush=True,
            )
...
async def get_workflow_preview(workflow_id: str):
    ...
    raise HTTPException(
        status_code=404, detail="No HTML preview found for this workflow"
    )
...
async def install_pack(req: Dict[str, Any]):
    ...
    conflict_action = req.get(
        "conflict_action", "prompt"
    )  # prompt, skip, replace, merge
    ...
    try:
        ...
        if is_local_url:
            ...
            if os.path.exists(local_path):
                print(
                    f"🔗 [Marketplace] Local source detected. Reading from disk: {local_path}"
                )
                            ...
                            "existing_agents": [
                                {
                                    "id": e["id"],
                                    "name": e["name"],
                                    "namespace": e.get("namespace", e["source"]),
                                }
            ...
            if conflict_action == "skip":
                ...
                for match in conflicts["exact_matches"]:
                    ...
                    if os.path.exists(conflict_path):
                        ...
                        print(
                            f"⊘ [Marketplace] Skipping conflicting file: {conflict_file}"
                        )
                ...
                for match in conflicts["exact_matches"]:
                    for existing in match["existing_agents"]:
                        ...
                        if existing_path and os.path.exists(existing_path):
                            ...
                            print(
                                f"🔄 [Marketplace] Replacing existing agent: {existing['id']}"
                            )
        ...
        skipped_count = (
            len(conflicts["exact_matches"]) if conflict_action == "skip" else 0
        )
...
async def uninstall_pack(req: Dict[str, str]):
    ...
    if os.path.exists(pack_dir):
        ...
        return {
            "status": "success",
            "message": f"Pack '{pack_id}' removed and archived.",
...
async def get_pack_agents(pack_id: str):
    ...
    return {"pack_id": pack_id, "agent_count": len(agents), "agents": agents}
...
async def get_namespace_stats():
    ...
    return {"stats": stats, "total_agents": sum(stats.values())}
...
async def delete_agent(agent_id: str):
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=404, detail="Agent not found or not deletable."
            )
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def update_pack(pack_id: str):
    ...
    if not os.path.exists(meta_path):
        raise HTTPException(
            status_code=404, detail="Installed pack metadata not found."
        )
    ...
    archive_dir = os.path.join(
        "data/agents/archive", pack_id, f"v{local_meta['version']}"
    )
    os.makedirs(archive_dir, exist_ok=True)
    ...
    await install_pack(
        {
            "pack_id": pack_id,
            "download_url": remote_pack["download_url"],
            "version": remote_pack["version"],
        }
    )

    return {
        "status": "success",
        "old_version": local_meta["version"],
        "new_version": remote_pack["version"],
    }
...
async def rollback_pack(pack_id: str, data: Dict[str, str]):
    ...
    if not os.path.exists(archive_dir):
        raise HTTPException(
            status_code=404, detail=f"Archive for version {version} not found."
        )
...
async def export_to_zip(req: Dict[str, Any]):
    ...
    with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as z:
        if agent_id:
            ...
            if not os.path.exists(pack_dir):
                raise HTTPException(
                    status_code=404, detail="Pack folder not found locally."
                )
    ...
    return FileResponse(
        path=export_path, filename=download_name, media_type="application/zip"
    )


# --- 🌐 Phase 2: Remote Marketplace Integration ---

from backend.ensemble.integrations.auto_update_service import auto_update_service
from backend.ensemble.integrations.github_pack_builder import GitHubPackBuilder
from backend.ensemble.integrations.marketplace_sync import marketplace_sync
...
async def list_remote_packs(source_id: Optional[str] = None):
    ...
    try:
        if source_id:
            ...
            source = next(
                (s for s in marketplace_sync.sources if s.id == source_id), None
            )
            if not source:
                raise HTTPException(
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch remote packs: {str(e)}"
        )
...
async def check_pack_updates(pack_id: str):
    ...
    for source in marketplace_sync.sources:
        ...
        try:
            update_info = source.check_for_updates(
                pack_id, local_meta.get("version", "1.0.0")
            )
            if update_info.get("has_update"):
                updates.append({"source": source.name, **update_info})
        except Exception as e:
...
async def list_marketplace_sources():
    ...
    return {"sources": marketplace_sync.get_source_status()}
...
async def remove_marketplace_source(source_id: str):
    ...
    try:
        ...
        return {"status": "success", "message": f"Source removed: {source_id}"}
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Failed to remove source: {str(e)}"
        )
...
async def get_workflow_outputs():
    ...
    try:
        ...
        with sqlite3.connect(audit_logger.db_path) as conn:
                ...
                if action_type == "RESULT" and result_content:
                    ...
                    if agent_id not in workflow_outputs:
                        ...
                        workflow_outputs[agent_id] = {
                            ...
                            "task": details.get(
                                "task", details.get("instruction", "Workflow execution")
                            )[:200],
                            "agentCount": 1,
                        }
            ...
            return {"outputs": workflow_outputs, "total": len(workflow_outputs)}
                    ...
                    results.append(
                        {
                            ...
                            "output": {"markdown": markdown},
                            "completedAt": timestamp,
                            "task": details.get("task", details.get("instruction", ""))[
                                :200
                            ],
                        }
                    )
...
async def set_provider_settings(req: Dict[str, Any]):
    ...
    try:
        ...
        result = settings.switch_provider(
            provider=provider, model=model, base_url=base_url, llm_instance=llm
        )
...
async def create_org(request: Request):
    ...
    try:
        ...
        org_id = (
            data.get("id")
            or data.get("name", "org").lower().replace(" ", "-")
            + "-"
            + str(uuid.uuid4())[:8]
        )
                    ...
                    "agents": [
                        {
                            ...
                            "emoji": "🎨",
                            "model": "gemini-2.5-flash",
                            "skills": ["Design Systems", "Visual Design"],
                        },
                    ],
                },
                {
                    ...
                    "agents": [
                        {
                            ...
                            "emoji": "🎯",
                            "model": "gemini-2.5-flash",
                            "skills": ["GDScript", "Game Architecture"],
                        },
                    ],
                },
                {
                    ...
                    "agents": [
                        {
                            ...
                            "emoji": "🎮",
                            "model": "gemini-2.5-flash",
                            "skills": ["Mechanics Design", "Level Design"],
                        },
                        {
                            "name": "Narrative Architect",
                            "role": "Game Narrative Architect",
                            "emoji": "📖",
                            "model": "gemini-2.5-flash",
                            "skills": ["Story Design", "Dialogue"],
                        },
                    ],
                },
            ],
                    ...
                    "agents": [
                        {
                            ...
                            "emoji": "💎",
                            "model": "gemini-2.5-flash",
                            "skills": ["Full-Stack Development", "Architecture"],
                        },
                    ],
                },
            ],
...
async def generate_company(request: Request):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to generate company: {str(e)}"
        )
...
async def get_user_settings_endpoint(request: Request):
    ...
    try:
        from backend.ensemble.settings import get_user_settings
...
async def save_user_settings_endpoint(request: Request):
    ...
    try:
        data = await request.json()
        from backend.ensemble.settings import save_user_settings
...
async def list_api_keys(request: Request):
    ...
    try:
        ...
        result = (
            ...
            .select("id,provider,key_suffix,is_active,last_used_at,created_at")
            .eq("user_id", user_id)
            .execute()
        )
...
async def add_api_key(request: Request):
    ...
    try:
        ...
        if not provider or not api_key:
            raise HTTPException(
                status_code=400, detail="provider and api_key are required"
            )

        from backend.ensemble.security.crypto import encrypt_api_key, mask_key
        from backend.ensemble.integrations.supabase_client import supabase_admin
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to save API key: {result.error}"
        )
...
async def list_api_keys(request: Request):
    ...
    try:
        ...
        result = supabase_admin.query(
            "user_api_keys",
            "select",
            columns="id,provider,key_suffix,is_active,last_used_at,created_at",
            eq="user_id",
            eq_value=user_id,
        )
...
async def delete_api_key(key_id: str, request: Request):
    ...
    try:
        ...
        result = supabase_admin.query(
            "user_api_keys", "delete", eq="id", eq_value=key_id
        )
...
async def test_api_key(request: Request):
    ...
    try:
        ...
        if not api_key:
            ...
            from backend.ensemble.security.crypto import decrypt_api_key
            from backend.ensemble.integrations.supabase_client import supabase_admin
            ...
            if result.data:
                ...
                raise HTTPException(
                    status_code=400, detail=f"No active {provider} API key found"
                )
        ...
        async with httpx.AsyncClient(timeout=15) as hc:
            if provider == "gemini":
                ...
                raise HTTPException(
                    status_code=400,
                    detail=f"Unsupported provider for testing: {provider}",
                )
        ...
        if resp.status_code == 200:
            ...
            return {
                "status": "error",
                "message": f"API key test failed: {resp.text[:200]}",
                "success": False,
            }
...
async def get_dashboard_stats(request: Request):
    ...
    try:
        ...
        with sqlite3.connect(audit_logger.db_path) as conn:
            ...
            cursor = conn.execute(
                "SELECT COUNT(*) FROM executions WHERE status = 'active'"
            )
            ...
            # Scheduled Jobs
            cursor = conn.execute(
                "SELECT COUNT(*) FROM scheduled_jobs WHERE status = 'pending' AND next_run >= date('now')"
            )
            ...
            cursor = conn.execute(
                "SELECT SUM(cost_usd) FROM events WHERE timestamp >= date('now')"
            )
            ...
            # Monthly Cost
            cursor = conn.execute(
                "SELECT SUM(cost_usd) FROM events WHERE timestamp >= date('now', 'start of month')"
            )
            monthly_cost = cursor.fetchone()[0] or 0.0
            ...
            # Success Rate
            total_runs = execution_stats.get("completed", 0) + execution_stats.get(
                "failed", 0
            )
            success_rate = round(
                (execution_stats.get("completed", 0) / max(total_runs, 1)) * 100, 1
            )
            ...
            return {
                ...
                "total_workflows": execution_stats.get("completed", 0)
                + active_workflows,
                "execution_stats": execution_stats,
    ...
    try:
        with sqlite3.connect(audit_logger.db_path) as conn:
            ...
            for row in cursor.fetchall():
                ...
                if any(
                    x in low_id
                    for x in ["step", "task_", "node_", "pipeline_", "trigger_"]
                ):
                    continue
                if len(agent_id) <= 2:
                    continue
                ...
                is_official = (
                    agent_id.startswith("agency_")
                    or agent_id.startswith("native_")
                    or agent_id.startswith("integration_")
                )
                ...
                stats.append(
                    {
                        ...
                        "category": (
                            agent_info.get("category") if agent_info else "Specialist"
                        ),
                        "runs": row[1],
...
async def create_scheduled_job(req: Dict[str, Any]):
    ...
    try:
        ...
        if not workflow_id or not next_run:
            raise HTTPException(
                status_code=400, detail="workflow_id and next_run are required"
            )

        scheduler.schedule_job(
            name, workflow_id, next_run, cron_pattern, req.get("payload")
        )
