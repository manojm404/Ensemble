...
from backend.ensemble.api.auth import UserInToken, get_current_user
from backend.ensemble.integrations.supabase_client import supabase_admin, verify_connection
...
async def signup(req: SignupRequest):
    ...
    try:
        ...
        profile_result = (
            client.table("profiles").select("*").eq("id", response.user.id).execute()
        )
...
async def refresh_token(req: RefreshTokenRequest):
    ...
    try:
        ...
        if token_response.status_code != 200:
            logger.warning(
                "⚠️ [Auth] Refresh token rejected by Supabase: %s", token_response.text
            )
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
...
async def forgot_password(req: ForgotPasswordRequest):
    ...
    try:
        ...
        redirect_url = os.getenv(
            "SUPABASE_PASSWORD_RESET_URL", "http://localhost:3000/auth/reset-password"
        )
        ...
        logger.warning(
            "⚠️ [Auth] Password reset request for non-existent email: %s", req.email
        )
...
async def reset_password(req: ResetPasswordRequest):
    ...
    try:
        ...
        if response.user:
            logger.info(
                "✅ [Auth] Password reset successful for user: %s", response.user.email
            )
            return MessageResponse(
                status="success",
...
async def get_current_user_profile(user: UserInToken = Depends(get_current_user)):
    ...
    try:
        ...
        result = supabase_admin.query(
            "profiles", "select", columns="*", eq="id", eq_value=user.id
        )
...
async def update_current_user_profile(
    ...
    try:
        ...
        result = supabase_admin.query(
            "profiles", "update", data=update_data, eq="id", eq_value=user.id
        )
