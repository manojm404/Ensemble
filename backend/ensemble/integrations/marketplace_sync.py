...
class MarketplaceSource:
    ...
    def _fetch_from_github(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            print(
                f"✅ [MarketplaceSync] Transformed {len(packs)} packs from {self.repo}"
            )
            return packs
    ...
    def _fetch_manifest(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"⚠️ [MarketplaceSync] Manifest not found (HTTP {response.status_code})"
                )
                return None
        except Exception as e:
    ...
    def _transform_plugin_to_pack(
        ...
        try:
            ...
            if not agent_files:
                print(
                    f"⚠️ [MarketplaceSync] No agent files found in plugin: {plugin_name}"
                )
                return None
            ...
            print(
                f"❌ [MarketplaceSync] Failed to transform plugin {plugin.get('name', 'unknown')}: {e}"
            )
            return None
    ...
    def check_for_updates(self, pack_id: str, local_version: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"has_update": False, "error": str(e), "pack_id": pack_id}
...
class MarketplaceSync:
    ...
    def check_all_updates(
        self, installed_packs: Dict[str, str]
    ) -> List[Dict[str, Any]]:
