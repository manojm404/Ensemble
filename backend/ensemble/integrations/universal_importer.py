...
from backend.ensemble.parsers.format_detector import FormatType, detect_repo_clone
from backend.ensemble.storage.pack_builder import PackBuilder
from backend.ensemble.parsers.agent_data import AgentCategory, AgentData
from backend.ensemble.parsers.all_parsers import JSONParser
from backend.ensemble.parsers.all_parsers import MarkdownParser
from backend.ensemble.parsers.all_parsers import PythonParser
from backend.ensemble.parsers.all_parsers import TextParser
from backend.ensemble.parsers.all_parsers import YAMLParser
...
class UniversalImporter:
    ...
    def _run_import(self, job: ImportJobStatus, callback: Optional[Callable]):
        """
        ...
        try:
            ...
            job.progress = 100.0
            job.message = (
                f"Import complete: {len(all_agents)} agents, {len(packs)} packs"
            )
            job.completed_at = datetime.utcnow().isoformat() + "Z"
    ...
    def _finalize_job(self, job: ImportJobStatus, callback: Optional[Callable]):
    ...
    def _clone_repo(self, repo_url: str, branch: str, job: ImportJobStatus) -> str:
        """
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to clone repository from {clone_url}: {e.stderr}"
            )
