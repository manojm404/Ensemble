...
class LLMProvider:
    ...
    def reinitialize(
        ...
        print(
            f"🔄 [LLMProvider] Reinitialized: {self.provider} | Model: {self.model} | URL: {self.base_url}",
            flush=True,
        )
        ...
        **kwargs,
        ...
        if self.provider == "gemini":
            ...
            return await self._chat_openai_compatible(
                messages, agent_name=agent_name, **kwargs
            )
        else:
    ...
    async def chat_with_model(
        ...
        **kwargs,
        ...
        try:
            ...
            print(
                f"🎯 [LLMProvider] Using model override: {override_provider}/{override_model}",
                flush=True,
            )
    ...
    async def chat_stream(
        ...
        **kwargs,
        ...
        if self.provider == "gemini":
            async for chunk in self._chat_gemini_stream(
                messages, agent_name=agent_name, **kwargs
            ):
                yield chunk
        elif self.provider in ["ollama", "openai", "local"]:
            async for chunk in self._chat_openai_compatible_stream(
                messages, agent_name=agent_name, **kwargs
            ):
                yield chunk
        else:
    ...
    def _prepare_messages(self, messages: List[Dict[str, str]]) -> List[Dict[str, str]]:
        ...
        if estimate(refined) > limit:
            print(
                f"⚠️ [LLMProvider] Context over limit, truncating older history...",
                flush=True,
            )
        ...
        "read_artifact": {
            ...
            "parameters": {
                ...
                "properties": {
                    "path": {
                        "type": "string",
                        "description": "The path or filename to read (e.g., 'data.xlsx')",
                    },
                    "file_id": {
                        "type": "string",
                        "description": "The unique ID of the file to read (from attachment metadata)",
                    },
                },
                "required": ["path"],
            },
        ...
        "write_artifact": {
            ...
            "parameters": {
                ...
                "properties": {
                    "path": {
                        "type": "string",
                        "description": "The path or filename to write to",
                    },
                    "content": {
                        "type": "string",
                        "description": "The string content or data to write",
                    },
                    "is_binary": {
                        "type": "boolean",
        ...
        **kwargs,
        ...
        while idx < len(messages):
            ...
            if role == "assistant" or role == "model":
                ...
                fcs = m.get("function_calls") or (
                    [m["function_call"]] if "function_call" in m else []
                )
                for fc in fcs:
        ...
        payload = {
            ...
            "generationConfig": {
                ...
                "maxOutputTokens": kwargs.get(
                    "max_tokens", 32768
                ),  # Increased for full HTML/code output
            },
        }
        ...
        async with httpx.AsyncClient(timeout=300.0) as client:
            try:
                ...
                if not candidates:
                    ...
                    return {
                        "text": "Error: Gemini returned no candidates (Response may be blocked).",
                        "usage": {},
                    }
                ...
                if not parts:
                    if finish_reason == "UNEXPECTED_TOOL_CALL":
                        print(
                            f"❌ [Protocol Culprit] UNEXPECTED_TOOL_CALL detected. Trace: {json.dumps(first, indent=2)}"
                        )
                    return {
                        "text": f"Error: Gemini response incomplete (Finish Reason: {finish_reason}).",
                        "usage": {},
                    }
                ...
                if tool_calls:
                    ...
                    print(
                        f"🛠️ [ParallelCall] Agent requested {len(tool_calls)} tools. Preamble: {preamble}"
                    )
                    ...
                    for fc in tool_calls:
                        ...
                        try:
                            from backend.ensemble.tools import execute_tool
                    ...
                    print(
                        f"🔄 [AgentMode] Re-calling LLM with {len(tool_calls)} results"
                    )
                    return await self._chat_gemini(
                        messages, agent_name=agent_name, **kwargs
                    )
        ...
        **kwargs,
        ...
        async with httpx.AsyncClient(timeout=300.0) as client:
            try:
                ...
                if not choices:
                    return {
                        "text": f"Error: No responses from {self.provider}.",
                        "usage": {},
                    }
