...
class GitHubPackBuilder:
    ...
    def build_multi_plugin_zip(self, plugin_names: List[str]) -> Optional[io.BytesIO]:
        ...
        try:
            print(
                f"📦 [PackBuilder] Building multi-plugin ZIP with {len(plugin_names)} plugins"
            )
    ...
    def _fetch_directory(self, path: str, files: List[Tuple[str, str]]):
        ...
        try:
            ...
            if response.status_code != 200:
                print(
                    f"⚠️ [PackBuilder] Failed to fetch {path} (HTTP {response.status_code})"
                )
                return
...
def create_pack_from_github(
    repo: str, plugin_name: str, branch: str = "main"
) -> Optional[io.BytesIO]:
    """
