...
Usage:
    from backend.ensemble.settings import get_user_settings, save_user_settings
...
def get_user_settings(user_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        result = supabase_admin.query(
            "user_settings", "select", columns="*", eq="user_id", eq_value=user_id
        )
        ...
        logger.warning(
            "⚠️ [Settings] Failed to fetch user settings from Supabase: %s — using local fallback",
            e,
        )
        return _load_local_settings()
...
def save_user_settings(user_id: str, settings: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        result = supabase_admin.query(
            "user_settings", "upsert", data=db_settings, on_conflict="user_id"
        )
        ...
        logger.warning(
            "⚠️ [Settings] Failed to save user settings to Supabase: %s — using local fallback",
            e,
        )
        return _save_local_settings(settings)
...
def switch_provider(
    provider: str, model: str, base_url: Optional[str] = None, llm_instance=None
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        result = await llm_instance.chat(
            messages=[
                {
                    "role": "user",
                    "content": "Say 'Connection test successful' in 3 words or less.",
                }
            ],
