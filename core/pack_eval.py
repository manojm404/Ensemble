...
class PackEvaluator:
    ...
    def evaluate_pack(self, pack_path: str) -> Dict[str, Any]:
        ...
        if not agent_files:
            return {"error": "No agent files found", "score": 0, "badge": QualityBadge.FAIL.value}
    ...
    def _check_role_clarity(self, frontmatter: Dict, body: str) -> float:
        ...
        identity_markers = ['you are', 'your role', 'your mission', 'identity', 'expert in']
    ...
    def _detect_anti_patterns(self, frontmatter: Dict, body: str, filepath: str) -> List[AntiPattern]:
    ...
    def _generate_recommendations(self, scores: Dict[str, float], anti_patterns: List[AntiPattern]) -> List[str]:
        """Generate actionable recommendations."""
        recommendations = []
        
        if scores.get('token_efficiency', 100) < 70:
            recommendations.append("Consider condensing prompt to improve token efficiency")
        
        if scores.get('role_clarity', 100) < 70:
            recommendations.append("Add clearer role definition with identity, mission, and rules sections")
        ...
        if scores.get('uniqueness', 100) < 70:
            recommendations.append("Agent appears similar to existing agents - differentiate or merge")
