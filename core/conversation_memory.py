...
class ConversationBufferMemory:
    ...
    def __init__(self, max_messages: int = 100, user_id: str = None, session_id: str = None):
    ...
    def _load_from_supabase(self):
        ...
        try:
            ...
            result = client.table("conversations").select("*").eq("user_id", self.user_id).eq("session_id", self.session_id).execute()
            ...
            if result.data:
                ...
                if messages_data:
                    ...
                    logger.debug("📥 [ConversationMemory] Loaded %d messages from Supabase (user: %s)", len(self._messages), self.user_id)

        except Exception as e:
            logger.warning("⚠️ [ConversationMemory] Failed to load from Supabase: %s", e)
    ...
    def _save_to_supabase(self):
        ...
        try:
            ...
            total_tokens = sum(len(m.content.split()) * 1.3 for m in self._messages)  # Rough estimate
            ...
            client.table("conversations").upsert(conv_data, on_conflict="user_id,session_id").execute()
