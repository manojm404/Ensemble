...
class SOPEngine:
    ...
    async def run_workflow(self, sop_path: str, company_id: str = "company_alpha", run_id: str = None, initial_input: str = None, assistant_id: str = None, topic_id: str = None):
        ...
        if initial_input and not run_id:
            self.space.write(initial_input.encode(), "user_initial_input", "start", company_id)
        
        # 0. Support Resume
        if run_id:
            with sqlite3.connect(self.gov.db_path) as conn:
                cursor = conn.execute("SELECT current_state, last_agent_id FROM sop_runs WHERE run_id = ?", (run_id,))
                row = cursor.fetchone()
                if row:
        ...
        if not self.current_state:
            self.current_state = workflow_cfg.get("initial_state", list(states.keys())[0])
            ...
            if initial_input:
                ...
                print(f"📥 Engine: Recorded user input: {initial_input[:50]}...", flush=True)

        print(f"🚀 Engine: Starting workflow {run_id} at state {self.current_state}", flush=True)
        ...
        while self.current_state:
            ...
            role_name = assistant_id if assistant_id and self.current_state == list(states.keys())[0] else state_config["role"]
            ...
            self._update_run_status(run_id, "RUNNING", self.current_state, agent.agent_id)
            ...
            if "Execution aborted" in response or "Budget exhausted" in response:
                ...
                self._update_run_status(run_id, "HIBERNATING", self.current_state, agent.agent_id)
                break
            ...
            if state_config.get("breakpoint_after"):
                self._save_state(agent, sop_path)
                self._update_run_status(run_id, "PAUSED", self.current_state, agent.agent_id)
    ...
    def _update_run_status(self, run_id: str, status: str, state: str = None, agent_id: str = None):
        ...
        with sqlite3.connect(self.gov.db_path) as conn:
            if state and agent_id:
                ...
                conn.execute("UPDATE sop_runs SET status = ? WHERE run_id = ?", (status, run_id))
    ...
    def _handle_outputs(self, outputs_config: List[Dict[str, Any]], response: str):
        ...
        for artifact_cfg in outputs_config:
            ...
            self.space.write(response.encode(), name, self.current_state, self.company_id)
    ...
    def _save_state(self, agent: ManagedAgent, sop_path: str):
        ...
        with open(save_path, "wb") as f:
            f.write(state_data)
        self.audit.log(self.company_id, agent.agent_id, "PAUSE", {"sop_path": sop_path, "state": self.current_state})
